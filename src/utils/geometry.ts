 ...
 */
function getBordersSize(styles: CSSStyleDeclaration, ...positions: Position[]) {
  return positions.reduce((size, position) => {
    const value =
        styles[`border-${position}-width` as string as 'borderTopWidth'];
...
const isSVGGraphicsElement = (() => {
  ...
  if (typeof SVGGraphicsElement !== 'undefined') {
    return (target: Element): target is SVGGraphicsElement =>
  ...
  return (target: Element): target is SVGGraphicsElement =>
             (target instanceof getWindowOf(target).SVGElement &&
              typeof (target as Partial<SVGGraphicsElement>).getBBox ===
                  'function');
})();
 ...
 *     dimensions.
 */
export function createReadOnlyRect({x, y, width, height}: DOMRectInit):
    DOMRectReadOnly {
...
export function createRectInit(
    x: number, y: number, width: number, height: number): DOMRectInit {
